...
// possible values for different modes
// inverter topology 0-10
// llc topology 0-2,5,8
// pv topology 0-3,8,9,10- signed as shutdown mode, 11 - normal mode
// buck topology 0-2
// did not produce the essence, although according to feng shui it is necessary...
...
enum  run_mode
{
  ...
  // followed by numerical values 11,12,13,14,15
};

/*
  * Inverter state.
  */
// this is c++... according to feng shui yes, not #define
...
struct inv8851_state_s {
     ...
     uint16_t data_size; // the number of bytes with data, similar to modbas here is always 144 / 2 = 72 words size
     ...
     union
     {
         ...
         struct
         {
             union // t0000
             {
                 uint16_t run_mode; // here are four fields of four bits each
                 struct
                 {
             ...
             int16_t t0019;
             int16_t t0020;
             int16_t inv_voltage; // 0.1 volts
             int16_t inv_current; // 0.01 a
             int16_t inv_freq; // 0.01 hz
             int16_t inv_va; // 1 va
             int16_t load_va; // 1 va
             int16_t t0026; // the same as inv_va only with a "-" sign, coincides with t0032, sometimes there is a difference of 2-3 units, it is similar to the power it consumes from the network, there is an opinion that it is full power
             ...
             int16_t t0032; // see t0026 is nowhere to be seen on the phone
             ...
             int16_t t0047;
             int16_t t0048; // there is some value here, but it is not displayed anywhere.
...
#pragma pack(push, 1)
struct inv8851_state_s_reverse {
     uint16_t crc;   // modbus crc16
     union
     {
         struct
...
struct inv8851_config_s {
    ...
    uint16_t command; // 0x0003 - read ? 0x0010 - write?
    uint16_t addr; // 0x0000 - for state, 0x0200 for config
    uint16_t data_size; // number of bytes with data, similar to modbas here 90/2=45 words size
